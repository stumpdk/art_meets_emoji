...
module.exports = {
    searchFavoritesRelatedImagesByText: function(userId, text, cb){
        ...
        pool.query({
            ...
            '(ra.title LIKE ? OR' +
            'author.name LIKE ? OR' +
            'ra.creation_date LIKE ?) ' +
            'AND reaction.positive = 1 AND reaction.user_id = ? AND seen_art.user_id is null GROUP BY ra.id LIMIT 1',
    ...
    searchFavoritesRelatedImages: function(userId, cb){
        pool.query({
            ...
            'JOIN  type ON ra.type_id = type.id' +
            'WHERE' +
            'reaction.positive = 1 AND reaction.user_id = ? AND seen_art.user_id is null GROUP BY ra.id LIMIT 1',
            values: [userId]
        }, function (error, results, fields) {
            if (error) throw error;
            console.log('The solution is: ', results);
